...
class DualHeadDeBERTa(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, labels_binary=None, **kwargs):
        ...
        if labels is not None and labels_binary is not None:
            loss_multi = F.cross_entropy(multi_logits, labels)
            loss_binary = F.cross_entropy(binary_logits, labels_binary)
...
class DualHeadTrainer(Trainer):
    """
    Trainer that handles the dual-head model.
    The model itself computes combined loss in forward(), so we just
    extract it. compute_metrics splits the concatenated logits back apart.
    """
