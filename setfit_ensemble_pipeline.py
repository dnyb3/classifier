...
Architecture — four independent classification perspectives:
  ...
  Perspective 1 — SetFit head:
    Classification head retrained per fold on frozen contrastive embeddings.

  Perspective 2 — Embedding ensemble (SVM, LR, XGBoost):
    Trained on the same frozen SetFit embeddings with different decision boundaries.

  Perspective 3 — TF-IDF word n-gram model:
    TF-IDF (1,3)-grams -> calibrated LR. Completely independent of SetFit.
    Catches surface-level regulatory phrases and negation collocations.

  Perspective 4 — Character n-gram model:
    TF-IDF char (3,5)-grams -> calibrated linear SVM. Independent of everything.
    Robust to ASR misspellings in messy transcripts.

  Final blend -> isotonic calibration -> calibrated probabilities.

Training flow:
  1. Train SetFit body ONCE on all data (contrastive learning)
  2. K-fold: retrain only heads/classifiers per fold using frozen body
     -> all folds share the same embedding space
     -> OOF predictions are coherent for calibration
  3. Fit calibrator on pooled OOF blended probabilities
  4. Final model: holdout eval then retrain classifiers on 100% data
...
class PipelineConfig:
    ...
    # SetFit body (contrastive learning — trained once)
    ...
    # Final model holdout for evaluation
...
def augment_and_balance(df, config):
...
def _get_setfit_probs(model, texts):
...
# ═══════════════════════════════════════════════════════════════════════════
# Perspective 1: SetFit — Body (once) and Head (per fold)
# ═══════════════════════════════════════════════════════════════════════════

def train_setfit_body(df, config):
    """
    Train the SetFit sentence transformer body with contrastive learning.
    This runs ONCE on all data. The resulting embedding space is frozen
    and reused for all subsequent training (k-fold heads, ensemble, final).

    Contrastive learning teaches the body to push same-class texts together
    and different-class texts apart. This is a representation task, not a
    decision-boundary task, so using all labels here is acceptable — the
    actual classification decisions are properly cross-validated downstream.
    """
    logger.info(f"Training SetFit body (contrastive learning on all {len(df)} samples)")
    logger.info(f"  Base model: {config.setfit_base_model}")
    ...
    train_dataset = Dataset.from_dict({
        "text": df[config.passage_col].tolist(),
        "label": df[config.label_col].tolist(),
    })

    training_args = SetFitTrainingArguments(
        output_dir=os.path.join(config.output_dir, "body_checkpoints"),
    ...
    trainer = SetFitTrainer(
        ...
        train_dataset=train_dataset, metric="f1",
    )
    trainer.train()

    logger.info("  Body training complete")
    return model


def retrain_setfit_head(model, train_embeddings, train_labels, val_embeddings=None, val_labels=None):
    """
    Retrain only the SetFit classification head on pre-computed embeddings.
    The body (sentence transformer) stays frozen — same embedding space everywhere.

    The SetFit model_head is a sklearn-compatible classifier (LogisticRegression
    by default). Calling .fit() replaces its weights without touching the body.
    After this, model.predict_proba(texts) will encode with the frozen body
    and classify with the newly-trained head.
    """
    model.model_head.fit(train_embeddings, train_labels)

    if val_embeddings is not None and val_labels is not None:
        preds = model.model_head.predict(val_embeddings)
        acc = accuracy_score(val_labels, preds)
        f1 = f1_score(val_labels, preds, average='macro', zero_division=0)
        logger.info(f"      setfit_head: acc={acc:.3f}, f1={f1:.3f}")


def encode_with_body(model, texts, batch_size=256):
    """Encode texts using the frozen SetFit body."""
...
def train_embedding_ensemble(X_train, y_train, X_val=None, y_val=None, config=None):
    """Train SVM, LR, XGBoost on frozen SetFit embeddings."""
...
def train_tfidf_word_model(train_texts, y_train, val_texts=None, y_val=None, config=None):
    """
    TF-IDF word (1,3)-grams -> calibrated LR. Completely independent of SetFit.
    Catches regulatory phrases, negation collocations, specific compliance language.
    """
    config = config or PipelineConfig()
    logger.info("    Training TF-IDF word n-gram model...")

    vectorizer = TfidfVectorizer(
        analyzer='word', ngram_range=config.tfidf_word_ngram_range,
    ...
    if val_texts is not None and y_val is not None:
        preds = clf.predict(vectorizer.transform(val_texts))
...
def train_char_ngram_model(train_texts, y_train, val_texts=None, y_val=None, config=None):
    """
    TF-IDF char (3,5)-grams -> calibrated LinearSVC. Independent of everything.
    Robust to ASR misspellings: "recommend"/"recomend" share 5/7 char trigrams.
    """
    config = config or PipelineConfig()
    logger.info("    Training character n-gram model...")

    vectorizer = TfidfVectorizer(
        analyzer='char_wb', ngram_range=config.tfidf_char_ngram_range,
        max_features=config.tfidf_char_max_features, sublinear_tf=True, min_df=2,
    )
    X_train = vectorizer.fit_transform(train_texts)
...
class MulticlassCalibrator:
    ...
    Isotonic > Platt because the blend of multiple model types can produce
    ...
    def fit(self, blended_probs, true_labels):
        ...
        for c in range(self.num_labels):
            ...
            iso.fit(blended_probs[:, c], y_binary)
...
class CombinedScorer:
    ...
    def score_batch(self, texts):
        ...
        # Agreement across independent perspectives
...
def _train_classifiers_on_split(
    body_model, train_texts, y_train, val_texts, y_val, config,
):
    """
    Train all 4 classification perspectives using a frozen SetFit body.
    Returns blended val probabilities and per-perspective val probs.
    """
    # Encode once with frozen body
    logger.info("  Encoding with frozen body...")
    X_train_emb = encode_with_body(body_model, train_texts)
    X_val_emb = encode_with_body(body_model, val_texts)

    # Perspective 1: SetFit head (retrain on these embeddings)
    logger.info("  Training SetFit head...")
    retrain_setfit_head(body_model, X_train_emb, y_train, X_val_emb, y_val)
    ...
    # Perspective 3: TF-IDF word (independent of body)
    ...
    setfit_probs = _get_setfit_probs(body_model, val_texts)
    ens_probs = _embedding_ensemble_proba(emb_ensemble, X_val_emb)
    tfidf_probs = tfidf_word_clf.predict_proba(tfidf_word_vec.transform(val_texts))
    char_probs = char_clf.predict_proba(char_vec.transform(val_texts))
    blended = blend_all_probas(setfit_probs, ens_probs, tfidf_probs, char_probs, config)

    # Return models for final training (None for k-fold where we discard them)
    trained_models = {
        'emb_ensemble': emb_ensemble,
        'tfidf_word': (tfidf_word_vec, tfidf_word_clf),
        'char_ngram': (char_vec, char_clf),
    }

    return blended, trained_models


def _report_metrics(y_true, blended_probs, label_names, calibrator=None, prefix=""):
    """Compute and log metrics from blended probs, optionally with calibration."""
    if calibrator:
        blended_probs = calibrator.transform(blended_probs)
    ...
    logger.info(f"\n  {prefix}Results: acc={metrics['accuracy']:.3f}, "
                f"f1_macro={metrics['f1_macro']:.3f}, precision={metrics['precision_macro']:.3f}, "
                f"recall={metrics['recall_macro']:.3f}")
    logger.info(f"\n{classification_report(y_true, preds, target_names=list(label_names.values()), zero_division=0)}")
...
def train_pipeline(config):
    ...
    Full training pipeline:
      ...
      1. Train SetFit body ONCE on all augmented data (contrastive learning).
         This creates a frozen embedding space shared by everything downstream.
      2. K-fold: retrain only classifiers (SetFit head, embedding ensemble,
         TF-IDF, char) per fold. Collect OOF blended probs.
      3. Fit calibrator on pooled OOF blended probs.
      4. Final model:
         a. Hold out a stratified fraction for evaluation
         b. Train classifiers on the remainder
         c. Evaluate on holdout (pre- and post-calibration)
         d. Retrain classifiers on 100% for production
         e. Save everything
    """
    logger.info("=" * 60)
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Step 1: Train SetFit body ONCE
    # ══════════════════════════════════════════════════════════════════════
    logger.info(f"\n{'='*60}")
    logger.info("STEP 1: SetFit Body (contrastive learning — one time)")
    logger.info("=" * 60)

    body_save_path = os.path.join(config.output_dir, "body")

    if os.path.exists(body_save_path):
        logger.info(f"  Body already trained, loading from {body_save_path}")
        body_model = SetFitModel.from_pretrained(body_save_path)
    else:
        aug_full = augment_and_balance(df, config)
        body_model = train_setfit_body(aug_full, config)
        body_model.save_pretrained(body_save_path)
        logger.info(f"  Body saved to {body_save_path}")
        del aug_full
        gc.collect()

    # ══════════════════════════════════════════════════════════════════════
    # Step 2: K-fold (classifiers only, frozen body)
    # ══════════════════════════════════════════════════════════════════════
    logger.info(f"\n{'='*60}")
    logger.info(f"STEP 2: K-FOLD VALIDATION ({config.n_folds} folds, frozen body)")
    logger.info("=" * 60)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(texts, labels)):
        ...
        aug_train_texts = aug_train_df[config.passage_col].tolist()
        y_train = aug_train_df[config.label_col].values
        val_texts_list = val_df[config.passage_col].tolist()
        y_val = val_df[config.label_col].values

        logger.info(f"  Train: {len(train_df)} -> {len(aug_train_df)} (aug) | Val: {len(val_df)}")

        # Train all classifiers with frozen body
        blended_val_probs, _ = _train_classifiers_on_split(
            body_model, aug_train_texts, y_train, val_texts_list, y_val, config
        )

        # Evaluate
        metrics, _ = _report_metrics(y_val, blended_val_probs, config.label_names,
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Step 3: Calibration (on pooled OOF predictions)
    # ══════════════════════════════════════════════════════════════════════
    logger.info(f"\n{'='*60}")
    ...
    if all_oof_probs:
        ...
        logger.warning("  No OOF predictions, skipping calibration")

    # ══════════════════════════════════════════════════════════════════════
    # Step 4: Final model — holdout eval then production retrain
    # ══════════════════════════════════════════════════════════════════════
    logger.info(f"\n{'='*60}")
    logger.info("STEP 4: FINAL MODEL")
    logger.info("=" * 60)
    ...
    holdout_metrics_path = os.path.join(final_dir, "holdout_metrics.json")

    all_saved = all(os.path.exists(p) for p in [setfit_save, emb_save, tfidf_save, char_save])
    ...
    if all_saved:
        ...
        if os.path.exists(holdout_metrics_path):
            with open(holdout_metrics_path, 'r') as f:
                hm = json.load(f)
            logger.info(f"  Holdout metrics: acc={hm.get('accuracy',0):.3f}, "
                        f"f1_macro={hm.get('f1_macro',0):.3f}")
    else:
        set_seed(config.seed)

        # 4a. Hold out a stratified fraction for evaluation
        holdout_frac = config.final_holdout_fraction
        if holdout_frac > 0 and holdout_frac < 1.0:
            train_main_df, holdout_df = train_test_split(
                df, test_size=holdout_frac, random_state=config.seed,
                stratify=labels,
            )
            logger.info(f"  Holdout split: {len(train_main_df)} train / {len(holdout_df)} holdout")
        else:
            train_main_df = df
            holdout_df = None
            logger.info("  No holdout (final_holdout_fraction=0)")

        # 4b. Train classifiers on train_main (augmented)
        aug_main = augment_and_balance(train_main_df, config)
        aug_main_texts = aug_main[config.passage_col].tolist()
        y_main = aug_main[config.label_col].values
        logger.info(f"  Training set: {len(train_main_df)} -> {len(aug_main)} (augmented)")

        if holdout_df is not None:
            holdout_texts = holdout_df[config.passage_col].tolist()
            y_holdout = holdout_df[config.label_col].values

            blended_holdout, _ = _train_classifiers_on_split(
                body_model, aug_main_texts, y_main, holdout_texts, y_holdout, config
            )

            # 4c. Evaluate on holdout
            logger.info(f"\n{'_'*40}")
            logger.info("HOLDOUT EVALUATION (pre-calibration)")
            logger.info(f"{'_'*40}")
            _report_metrics(y_holdout, blended_holdout, config.label_names,
                            prefix="Holdout pre-cal ")

            if calibrator:
                logger.info(f"\n{'_'*40}")
                logger.info("HOLDOUT EVALUATION (post-calibration)")
                logger.info(f"{'_'*40}")
                holdout_metrics, holdout_preds = _report_metrics(
                    y_holdout, blended_holdout, config.label_names,
                    calibrator=calibrator, prefix="Holdout post-cal "
                )
            else:
                holdout_metrics, holdout_preds = _report_metrics(
                    y_holdout, blended_holdout, config.label_names,
            ...
            logger.info("  Skipping holdout evaluation")

        # 4d. Retrain ALL classifiers on 100% data for production
        logger.info(f"\n{'_'*40}")
        logger.info("PRODUCTION MODEL (100% data)")
        logger.info(f"{'_'*40}")

        aug_all = augment_and_balance(df, config)
        all_texts = aug_all[config.passage_col].tolist()
        y_all = aug_all[config.label_col].values
        logger.info(f"  Full dataset: {len(df)} -> {len(aug_all)} (augmented)")

        # Encode all with frozen body
        logger.info("  Encoding all data with frozen body...")
        X_all_emb = encode_with_body(body_model, all_texts)

        # Retrain SetFit head on all data
        logger.info("  Training production SetFit head...")
        retrain_setfit_head(body_model, X_all_emb, y_all)
        body_model.save_pretrained(setfit_save)
        logger.info(f"  SetFit saved to {setfit_save}")

        # Embedding ensemble
        logger.info("  Training production embedding ensemble...")
        emb_ens = train_embedding_ensemble(X_all_emb, y_all, config=config)
        with open(emb_save, 'wb') as f:
            pickle.dump(emb_ens, f)

        # TF-IDF word
        logger.info("  Training production TF-IDF word model...")
        tv, tc = train_tfidf_word_model(all_texts, y_all, config=config)
        with open(tfidf_save, 'wb') as f:
            pickle.dump((tv, tc), f)

        # Char n-gram
        logger.info("  Training production char n-gram model...")
        cv, cc = train_char_ngram_model(all_texts, y_all, config=config)
    ...
    logger.info("=" * 60)
    logger.info(f"  Models saved to: {final_dir}")
    logger.info(f"  Files: setfit/, embedding_ensemble.pkl, tfidf_word_model.pkl, "
                f"char_ngram_model.pkl, calibrator.pkl")
